...
function recordQuality(r: ProductionRecord): {
  tone: "ok" | "warn";
  label: string;
  detail: string;
} {
  if (r.warningCodes.length > 0) {
    const detail = r.warningCodes
      .map((c) => WARNING_LABELS[c] ?? c)
      .join(", ");
    return { tone: "warn", label: "경고", detail };
  }
  return { tone: "ok", label: "정상", detail: "" };
}
...
export default function ProductionDataPage() {
  ...
  const { state, patch } = usePageState("production-data-v2", "date", "asc");
  ...
  const rows = useMemo(() => {
    ...
    const sorted = sortBy(list, state.sort, state.order, {
      date: (r) => r.workDate,
    ...
    return [...sorted].sort((a, b) => {
      const aw = a.warningCodes.length > 0 ? 0 : 1;
      const bw = b.warningCodes.length > 0 ? 0 : 1;
      return aw - bw;
    });
        ...
        {rows.length === 0 ? (
          ...
          <>
            <DataEditToolbar
              selectionLabel={
                selected
                  ? `${selected.workDate} · ${selected.equipmentName} · ${selected.partNumber}`
                  : null
              }
              emptyLabel={
                isAdmin
            ...
            >
              {selected ? (
                <span
                  className={`pd-status-badge pd-status-badge-${recordQuality(selected).tone}`}
                  title={recordQuality(selected).detail || undefined}
                >
                  {recordQuality(selected).label}
                </span>
              ) : null}
              {isAdmin ? (
                <DataHistoryButton onClick={() => setHistoryOpen(true)} />
              ) : null}
              <DataEditButton
                isAdmin={isAdmin}
                ...
                lockedTitle="생산 DATA 수정은 관리자 모드에서만 가능합니다."
                label="선택 행 수정"
                size="sm"
                tone="production"
              />
                    ...
                    >
                      ...
                      <td>
                        {(() => {
                          ...
                          return (
                            <span
                              className={`pd-status-badge pd-status-badge-${q.tone}`}
